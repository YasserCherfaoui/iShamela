...
- **SB-02** On successful enqueue: show `جارٍ تنزيل «{title}»`; bar indeterminate until `bytesTotal` known, then determinate, **streaming live progress** from the queue while visible. Action **عرض** → Downloads tab (النشطة). **Do not** show **افتح** during progress.
- **SB-03** While downloading, the snackbar stays visible (no auto-dismiss). A new enqueue while visible replaces content and coalesces the label: `جارٍ تنزيل {n} كتب` (count = currently active+queued), same **عرض** action.
- **SB-04** If the download completes while the snackbar is visible, it morphs in place to `اكتمل تنزيل «{title}»` with action **افتح** only (≈10 s so the user can open). If it fails while visible: error variant — leading glyph and action tinted `error`-on-dark (`#E08A76`), text `تعذر تنزيل «{title}»`, action **إعادة** re-enqueues (≈8 s).
