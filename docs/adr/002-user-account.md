...
3. **Cloud Firestore** stores only *user* data under `users/{uid}` (profile doc + `history`, `bookmarks`, `notes`, `highlights`, `progress`, and `library` — the installed-bookshelf manifest, SPEC-025 — subcollections). Security rules: `request.auth.uid == uid` on the whole subtree. Row volume per user is tiny (KBs). **Book content itself never transits the backend:** the `library` manifest only names books; devices download the files from HuggingFace as always.
...
- **New repo surface:** `firebase/` folder — `firestore.rules`, `firestore.indexes.json`, `functions/` (TypeScript: `sendOtp`, `verifyOtp`, `deleteAccount`), `firebase.json`. Client config is `flutterfire configure`, but API keys stay in gitignored files (`app/lib/firebase_local_secrets.dart`, `GoogleService-Info.plist`, `google-services.json`). Firestore rules remain the access boundary; keys still need Google Cloud application and API restrictions because they ship inside the app.
