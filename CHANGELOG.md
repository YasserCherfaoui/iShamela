...
- App foundation (SPEC-004): Flutter shell with Riverpod, catalog sync, browse/search (RTL ar/en/fr), download queue (max 2, Range resume), sha256 verify, zstd install via `zstandard` plugin. Task 0 evidence in `docs/ZSTD.md`. macOS Runner grants `network.client` (sandbox) and `NSAllowsLocalNetworking` for local catalog E2E.
